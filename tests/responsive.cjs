...
test("CONTENT_MAX_WIDTH: narrow (auth/forms) is meaningfully narrower than default (general content)", () => {
  const { narrow, default: defaultWidth } = responsive.CONTENT_MAX_WIDTH;
  assert.ok(narrow > 0 && defaultWidth > 0);
  assert.ok(narrow < defaultWidth);
  // A readable form width, not a full dashboard width.
  assert.ok(narrow >= 400 && narrow <= 600);
  // Roughly the 1100-1280px range called for general desktop content.
